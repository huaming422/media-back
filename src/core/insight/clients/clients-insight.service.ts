...
export class ClientsInsightService {
  ...
  private async getDiscoverClientsCountDataDataIncluded(
    ...
    queryWhere: Prisma.DiscoverClientWhereInput,
  ) {
    const dataIncluded: { GraphIncludeData?: number | 'N/A' } = {};
    ...
    for (const data of includeData) {
      ...
      if (data === GraphIncludeData.total) {
        dataIncluded[data] = (
          await this.prismaService.discoverClient.count({
            select: { _all: true },
            where: {
              ...graphQueryWhere(graphType, undefined, undefined),
              ...queryWhere,
        ...
        dataIncluded[data] = await (async () => {
          ...
          const lastData = await this.prismaService.discoverClient.count({
            select: { _all: true },
            where: {
              ...graphQueryWhere(
                graphType,
          ...
          const preLastData = await this.prismaService.discoverClient.count({
            select: { _all: true },
            where: {
              ...graphQueryWhere(
                graphType,
  ...
  async getDiscoverClientsCountData(
    ...
    { status }: DiscoverClientFilterParamsDto,
  ) {
    const queryWhere: Prisma.DiscoverClientWhereInput = {
      status,
    };
    const periods = useStrictPeriod
      ? getPeriods(
      ...
      : await (async () => {
          ...
          const discoverClient =
            await this.prismaService.discoverClient.aggregate({
              _min: { createdAt: true },
              where: { ...queryWhere },
            });
    ...
    for (const { dateFrom, dateTo } of periods) {
      const queryResult = await this.prismaService.discoverClient.count({
        select: { _all: true },
        where: {
          ...graphQueryWhere(graphType, dateFrom, dateTo),
          ...queryWhere,
